...
def main():
    ...
    for timestamp_ns in tqdm(img_timestamps_ns):
        ...
        if indexes_activation.size > 0: 
           ...
           for name in objects_that_moved_names:
                
                """
                1. Check if the object is already moving.
                2. If not, initialize a new movement period.
                3. If yes, update the movement for the current period.
                """

                # Get the current position of the user in the XZ plane
                
                for i, (name, index) in enumerate(zip(objects_that_moved_names, indexes_activation)):
                    
                    current_position = np.array([user_ema_position[0], 0, user_ema_position[2]])

                    # Object has just started moving
                    movement_counter[name] = movement_counter.get(name, 0) + 1  # Increment movement counter
                    movement_number = movement_counter[name]
                    unique_key = f"{name}_{movement_number}"
                    
                    # Initialize movement tracking for the new period
                    user_object_position[unique_key] = [current_position]
                    user_object_movement[unique_key] = 0
                    moving_objects[name] = movement_number
                    
                    # Record the start time
                    movement_time_dict_list.setdefault(name, {"start_times": [], "end_times": []})
                    movement_time_dict_list[name]["start_times"].append(current_time_s)

                    # **Initialize the end_times list by appending None**
                    movement_time_dict_list[name]["end_times"].append(None)
           
                    # Record the start time in movement_time_dict (single period per object)
                    movement_time_dict[name] = {"start_time": current_time_s, "end_time": None}

                    # Add to original_objects_that_moved_dict
                    original_objects_that_moved_dict[current_time_s] = name

                    # Update previous_moved_names and indexes_of_objects_that_moved
                    previous_moved_names.append(name)
                    indexes_of_objects_that_moved.append(index)
                    ...
                    movement_number = moving_objects[name]
                    unique_key = f"{name}_{movement_number}"

                    # Update position and movement
                    user_object_position[unique_key].append(current_position)
                    movement_vector_xz = user_object_position[unique_key][-1] - user_object_position[unique_key][-2]
                    user_object_movement[unique_key] += np.linalg.norm(movement_vector_xz)

                    # Update the end time in movement_time_dict_list
                    movement_time_dict_list[name]["end_times"][-1] = current_time_s

                    # Update the end time in movement_time_dict
                ...
                for name in movement_time_dict_list:
                    if name not in objects_that_moved_names:
                        # Append the end time if the object has stopped moving and it hasn't been recorded yet
                        if movement_time_dict_list[name]["end_times"][-1] is None:
                            movement_time_dict_list[name]["end_times"][-1] = current_time_s

                        # Remove the object from moving_objects
                        if name in moving_objects:
                            movement_number = moving_objects.pop(name)
                            unique_key = f"{name}_{movement_number}"

                            # Ensure end time is recorded in movement_time_dict
                            movement_time_dict[name]["end_time"] = current_time_s

                # # Identify objects that have stopped moving
                # objects_no_longer_moving = set(moving_objects.keys()) - set(objects_that_moved_names)
                # for name in objects_no_longer_moving:
                #     # Retrieve movement number and unique key
                #     movement_number = moving_objects.pop(name)
                #     unique_key = f"{name}_{movement_number}"

                #     # Ensure end time is recorded in movement_time_dict_list
                #     movement_time_dict_list[name]["end_times"][-1] = current_time_s
    ...
    # Movement of object X with start / end time
    with open(os.path.join(gt_folder,'movement_time_dict_list.json'), 'w') as json_file:
        json.dump(movement_time_dict_list, json_file)
