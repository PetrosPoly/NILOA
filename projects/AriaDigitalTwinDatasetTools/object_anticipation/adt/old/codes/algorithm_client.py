...
from projects.AriaDigitalTwinDatasetTools.object_anticipation.adt.old.codes.openai_models_openai_async_multi_client import ( 
...
async def execute_algorithm(parameters, paths):
        ...
        if (high_dot_counts_but_also_distance
            ...
            if args.use_llm and not llm_activated:  
                ...
                if any(object in high_dot_history for object in less_than_2_seconds_dict.keys()): # TODO: make this soft having it as a group of objects that was looking, Red Clock was within a group 
                    ...
                    # Call the `activate_llm` function from `openai_models_openai_async_multi_client.py`
                    processed_response = await activate_llm(log_content, parameters, paths['prompt'], current_time_s)

                    if processed_response:
                        # Destructure response if valid
                        objects_possibility_dict[timestamp_ns] = processed_response.get("objects_possibility")
                        rationale_dict[timestamp_ns] = processed_response.get("rationale")
                        predictions_dict[timestamp_ns] = processed_response.get("predicted_objects")
                        goals_dict[timestamp_ns] = processed_response.get("goal")
                        print(f"LLM activation was successful at timestamp {timestamp_ns}")
                    else:
                        print(f"LLM activation failed at timestamp {timestamp_ns}")
