...
from utils.rate_limit import AsyncRateLimiter, AsyncRateLimiter_per_second 
...
from projects.AriaDigitalTwinDatasetTools.object_anticipation.adt.utils.openai_models_async import ( 
...
# Adjusted activate_llm function (synchronous placeholder)
def prepare_llm_request(history_log_string, parameters):
    return history_log_string, parameters

def main(parameters, api_queue, result_dict):
        ...
        if (high_dot_counts_but_also_distance
            ...
            if args.use_llm and not llm_activated:  
                ...
                if any(object in high_dot_history for object in less_than_2_seconds_dict.keys()): # TODO: make this soft having it as a group of objects that was looking, Red Clock was within a group 
                    ...
                    request_id = f"{multiprocessing.current_process().pid}_{current_time_s}"

                    # Prepare the data for the API call
                    api_request_data = (history_log_string, parameters, request_id)

                    # Put the data into the API queue
                    api_queue.put(api_request_data)

                    # Wait for the result from the API worker
                    ...
                    if isinstance(result, Exception):
                        print(f"Error in API call: {result}")
                        logging.error(f"Error in API call: {result}")
                    else:
                        # Unpack the result
                        objects_possibility, rationale, predicted_objects, goal, llm_response = result
...
# ==============================================
# API Worker (I/O-bound Tasks)
# ==============================================
def api_worker(api_queue, result_dict):
    asyncio.run(api_loop(api_queue, result_dict))

async def api_loop(api_queue, result_dict):
    rate_limiter = AsyncRateLimiter_per_second(max_calls=500, period=60)
    total_api_calls = 0  # Local to the API worker process
    ...
    while True:
        ...
        history_log_string, parameters, request_id = item
            ...
            try:
                ...
                if llm_response:
                    ...
                    if any(v is None for v in [objects_possibility, rationale, predicted_objects, goal]):
                        ...
                        success = True
                        # Include llm_response in the result
                        result_dict[request_id] = (objects_possibility, rationale, predicted_objects, goal, llm_response)
                else:
                ...
                print(f"Error during LLM activation or parsing: {e}")
                logging.error(f"Error during LLM activation or parsing with parameters: {parameters}")
                retry_count += 1

        if not success:
            result_dict[request_id] = Exception("Exceeded maximum retries")
...
if __name__ == "__main__":
    ...
    # Start the API worker process
    api_process = Process(target=api_worker, args=(api_queue, result_dict))
    api_process.start()

    num_processes = cpu_count()
    print(f"Utilizing {num_processes} CPU cores")

    pool = Pool(processes=num_processes)

    # Prepare the arguments for each process
    tasks = [(params, api_queue, result_dict) for params in param_combinations]

    # Run the main function in parallel across multiple processes
    ...
    # Signal the API worker to exit
    api_queue.put(None)

    # Wait for the API worker to finish
    api_process.join()
