...
from algorithm import execute_algorithm, save_results

from utils.rate_limit import SharedRateLimiter, AsyncRateLimiter
...
high_dot_thresholds = [0.7] # [0.7, 0.8, 0.9]               # [0.5, 0.6, 0.7, 0.8, 0.9]                 
distance_thresholds = [1.5] # [1.5, 2, 2.5]                        
high_dot_counters_threshold = [45] # [15, 30, 45, 60, 75, 90]  
...
# This function Define the function that processes a batch of parameters within a process
def process_param_combinations(params_batch, data, rate_limiter):
    async def run_all_tasks():
        ...
        tasks = [execute_algorithm(parameters, data, rate_limiter) for parameters in params_batch]
        await asyncio.gather(*tasks)  # Run all tasks concurrently

    # Run the asyncio loop in this process
...
def main():
    ...
    # Step 2: Define a rate limiter (shared by all processes to respect API limits)
    rate_limiter = AsyncRateLimiter(max_calls=500, period=60)

    # Step 3: Split parameter combinations among processes
    params_batches = np.array_split(param_combinations, num_processes)

    # Step 4: Create processes without the result queue
    processes = [
        Process(target=process_param_combinations, args=(params_batch, data, rate_limiter))
