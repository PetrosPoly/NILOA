...
from projects.AriaDigitalTwinDatasetTools.object_anticipation.adt.utils.openai_models_async_multi import ( 
...
async def execute_algorithm(parameters, paths, rate_limiter):
            ...
            if args.use_llm and not llm_activated:  
                ...
                if any(object in high_dot_history for object in less_than_2_seconds_dict.keys()): # TODO: make this soft having it as a group of objects that was looking, Red Clock was within a group 
                    ...
                    while not success and retry_count < max_retries:
                        try:
                            # Respect the rate limiter before making an API call
                            await rate_limiter.acquire()

                            # Asynchronous API call
                            llm_result = await activate_llm(log_content, parameters, rate_limiter, paths['prompt'])

                            # If a response is received, parse and validate
                            if llm_result:
                                objects_possibility = llm_result.get("objects_possibility")
                                rationale = llm_result.get("rationale")
                                predicted_objects = llm_result.get("predicted_objects")
                                goal = llm_result.get("goal")

                                # Validate all expected components are present
                                if all([objects_possibility, rationale, predicted_objects, goal]):
                                    success = True  # Mark success to exit retry loop
                                else:
                                    print(f"Attempt {retry_count + 1}: Incomplete response from LLM.")
                                    retry_count += 1
                            else:
                                print(f"Attempt {retry_count + 1}: No response from LLM.")
                                retry_count += 1
                    ...
                    # Handle failed attempts after retries
                    if not success:
                        print(f"Failed to get a valid response from LLM after {max_retries} attempts.")
